...
# @Last Modified time: 2015-04-27 16:30:30
require 'colorize'
class BoolCalc
    ...
    def rotate_left()
        temp_string= ""
        temp_num = String(@x)
        temp_num = temp_num.chars.to_a
        temp_num = temp_num.rotate!(@y)
        temp_num.each do |charac|
            temp_string << charace
        end
        temp_num = Integer(temp_string)
        puts temp_num.class
        return temp_num
    end
    ...
    def parse_args()
        ops = ['NOT','SHR','SHL','AND','OR','XOR','RAD','ROL']
        i = 0
    ...
    def rol()
        new_string = ""
        to_rotate = @x.chars.to_a
        puts "ROL ARRAY: #{to_rotate}"
        to_rotate = to_rotate.rotate!(@y)
        to_rotate.each do |elem|
            new_string << elem.to_i
        end
        puts ne
